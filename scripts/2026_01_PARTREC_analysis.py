...
EXPERIMENT_NAME = "2026_01_PARTREC"
partrec_paths = ExperimentPaths(EXPERIMENT_NAME)


def partrec_2026_01_analysis():
    ### runs_df
    # load runs CSV
    runs_df = pd.read_csv(partrec_paths.runs_info_csv)
    ...
    sdc_details_df.to_csv(
        partrec_paths.sdc_details_csv, index=False, sep=",", encoding="utf-8"
    )
    print(f"Saved {partrec_paths.sdc_details_csv}")

    # create sdc_df
    sdc_df = create_sdc_df(sdc_details_df, EXPERIMENT_NAME)
    sdc_df.to_csv(partrec_paths.sdcs_csv, index=False, sep=",", encoding="utf-8")
    print(f"Saved {partrec_paths.sdcs_csv}")
    ...
    cross_section_df.to_csv(
        partrec_paths.cross_sections_per_run_csv, index=False, sep=",", encoding="utf-8"
    )
    print(f"Saved {partrec_paths.cross_sections_per_run_csv}")
    ...
    print(f"Saved {partrec_paths.cross_sections_per_model_csv}")
